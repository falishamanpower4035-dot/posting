#!/usr/bin/env python3
"""Check camel fair post posting history"""
import sys
import json
...
for post_id in sorted(all_posts):
    ...
    if 'camel' in title or 'pushkar' in title:
        camel_posts.append({
            'post_id': post_id,
...
print("="*70)
print("CAMEL FAIR POST ANALYSIS")
print("="*70)

if not camel_posts:
    print("\nNo camel fair posts found")
else:
    for post in camel_posts:
        print(f"\nPost {post['post_id']}:")
...
if schedule_file.exists():
    ...
    camel_schedules = [s for s in schedules if any(p['post_id'] == s.get('post_id') for p in camel_posts)]
    
    if camel_schedules:
        print(f"\n\nSchedules for camel fair posts:")
        for sched in sorted(camel_schedules, key=lambda x: x.get('scheduled_at', '')):
