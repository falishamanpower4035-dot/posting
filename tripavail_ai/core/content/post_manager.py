...
class PostManager:
    ...
    def archive_post(self, post_id: str, reason: str = "failed") -> bool:
        """
        Archive a post by moving it to an archive directory
        
        Args:
            post_id: Post identifier
            reason: Reason for archiving (e.g., "failed", "skipped")
        
        Returns:
            True if archived successfully, False otherwise
        """
        post_dir = self.base_dir / f"post_{post_id}"
        archive_dir = self.base_dir.parent / "posts_archive" / reason
        archive_dir.mkdir(parents=True, exist_ok=True)
        
        if not post_dir.exists():
            logger.warning(f"Post {post_id} directory not found, cannot archive")
            return False
        
        try:
            archived_path = archive_dir / f"post_{post_id}"
            if archived_path.exists():
                # If already archived, add timestamp
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                archived_path = archive_dir / f"post_{post_id}_{timestamp}"
            
            shutil.move(str(post_dir), str(archived_path))
            logger.info(f"Archived post {post_id} to {archived_path} (reason: {reason})")
            return True
        except Exception as e:
