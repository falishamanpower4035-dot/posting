...
Unsplash → Pexels → AI Fallback for authentic travel photography
"""
...
class HybridImageGenerator:
    ...
    def extract_keywords_and_create_query(self, post: Dict[str, Any], story_beat: str = None) -> str:
        """
        Step 2: Extract keywords and create search query
        NOW SUPPORTS STORY BEATS for precise image matching
        
        Args:
            post: Post data
            story_beat: Optional specific scene to search for (e.g. "vendor cooking at wok")
        """
        try:
            # Prefer structured locations/keywords if available
            locations = post.get('locations', {}) or {}
            kw = post.get('keywords', {}) or {}
            primary_kw = kw.get('primary', [])
            visual_kw = kw.get('visual', [])

            # If story beat provided, use it for highly specific search
            if story_beat:
                # Compose from locations and beat
                loc_parts = [locations.get('city', ''), locations.get('region', ''), locations.get('country', '' )]
                loc = ' '.join([p for p in loc_parts if p]).strip() or post.get('region', '')
                
                # Enhanced keyword matching for voiceover alignment
                narrative_kw = kw.get('narrative', [])
                secondary_kw = kw.get('secondary', [])
                
                # Combine visual, narrative, and secondary keywords for better matching
                all_visual_kw = visual_kw + narrative_kw + secondary_kw
                vis_hint = ' '.join(all_visual_kw[:3]) if all_visual_kw else ''  # Top 3 keywords
                
                # Create focused query from story beat with enhanced keywords
                query_parts = [loc, story_beat, vis_hint]
                query = ' '.join([p for p in query_parts if p]).strip()
                
                # Clean and optimize
                query = self._optimize_search_query(query)
                
                # ENHANCED: Use OpenAI to optimize query for better Shutterstock results
                query = self._optimize_query_with_openai(post, query)
                
                post_id = post.get('post_id') or post.get('topic_id', 'unknown')
                logger.info(f"Enhanced story beat query for post {post_id}: '{query}'")
                return query
            
            # Otherwise use original logic
            caption = post.get('caption', '')
            # Build location string from structured fields first
            loc_parts = [locations.get('city', ''), locations.get('region', ''), locations.get('country', '')]
            region = ' '.join([p for p in loc_parts if p]).strip() or post.get('region', '')
            ...
            location_keywords = self._extract_location_keywords(title, summary)
            keywords.extend(location_keywords)
            
            # Extract theme keywords
            theme_keywords = self._extract_theme_keywords(caption, title, summary)
            ...
            if location_keywords:
                query_parts.append(' '.join(location_keywords[:2]))  # Max 2 location terms
            elif region:
                query_parts.append(region)
            
            # Add theme
            # Use primary keywords first, then theme
            if primary_kw:
                query_parts.append(' '.join(primary_kw[:3]))
            if theme_keywords:
                query_parts.append(' '.join(theme_keywords[:3]))  # Max 3 theme terms
            
            # Add travel context
            query_parts.append('travel tourism destination')
            
            query = ' '.join(query_parts)
            
            # Clean and optimize query
            base_query = self._optimize_search_query(query)
            
            # ENHANCED: Use OpenAI to optimize query for better Shutterstock results
            optimized_query = self._optimize_query_with_openai(post, base_query)
            
            # Use optimized query, but fallback to base if optimized is too different or empty
            # (This prevents using queries that are too specific)
            if optimized_query and len(optimized_query.split()) >= 2:  # Allow 2+ words (simple is good!)
                query = optimized_query
            else:
                query = base_query
                logger.info(f"Using base query (optimized was too short/specific): '{query}'")
            
            # Get post ID (could be post_id or topic_id)
            post_id = post.get('post_id') or post.get('topic_id', 'unknown')
            logger.info(f"Created search query for post {post_id}: '{query}'")
            return query
            
        except Exception as e:
            logger.error(f"Error creating search query: {e}")
            # Fallback with more specific region-based query
            region = ' '.join([p for p in [locations.get('city',''), locations.get('region',''), locations.get('country','')] if p]) or post.get('region', 'travel')
            return f"{region} tourism destination landscape"
...
Generate the query now:"""
    ...
    def search_shutterstock(self, query: str, count: int = 3, page: int = 1, sort: str = "relevance") -> List[Dict[str, Any]]:
        """Step 2: Search Shutterstock (first priority) for real photos (supports paging and sort for diversity)."""
        try:
            if not self.shutterstock_access_token:
    ...
    def search_unsplash(self, query: str, count: int = 3, page: int = 1, order_by: str = "relevant") -> List[Dict[str, Any]]:
        """Step 3: Search Unsplash for real photos (supports paging for diversity)"""
        try:
    ...
    def search_pexels(self, query: str, count: int = 3, page: int = 1) -> List[Dict[str, Any]]:
        """Step 4: Search Pexels as fallback (supports paging for diversity)"""
        try:
...
[READY] Images are ready for Day 6 video assembly!"""
    ...
    def generate_for_post(self, post: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Search Shutterstock first (PRIORITY)
            shutter_results = self.search_shutterstock(query, target_count)
            try:
            ...
            # Search Pexels next
            if len(images_found) < target_count:
                pexels_results = self.search_pexels(query, target_count - len(images_found))
                try:
            ...
            # Search Unsplash last (stock fallback)
            unsplash_results = self.search_unsplash(query, target_count - len(images_found) if len(images_found) < target_count else 0)
    ...
    def generate_for_post_with_story_beats(self, post: Dict[str, Any], target_images_dir: Path) -> bool:
        ...
        try:
            ...
            for i, story_beat in enumerate(story_beats[:image_count], 1):
                ...
                # Try sources in order: Shutterstock -> Pexels -> Unsplash -> AI
                image_found = False
                
                # 1. Try Shutterstock (PRIORITY)
                    ...
                    if image_url:
                        ...
                        if processed_path and self.quality_check(processed_path):
                            ...
                            if ihash and ihash in existing_hashes:
                                logger.info("Duplicate image detected (hash match), skipping")
                                continue
                ...
                # 2. Try Pexels if Shutterstock failed
                if not image_found:
                    ...
                    for result in unsplash_results:
                        ...
                        if image_url:
                            ...
                            if processed_path and self.quality_check(processed_path):
                                ...
                                shutil.move(processed_path, str(dest_path))
                                images_found.append(str(dest_path))
                                logger.info(f"Found generic image {i}")
                                image_found = True
                                break
        ...
        try:
            ...
            for i, beat in enumerate(additional_beats, start=1):
                ...
                for page in range(1, 6):  # try up to 5 pages
                    if found_image:
                        break
                    # Try Unsplash first with varying order_by for diversity
                    ...
                    for result in pexels_results:
                        ...
                        if processed_path and self.quality_check(processed_path):
                            ...
                            if ihash:
                                existing_hashes.add(ihash)
                            logger.info(f"Found additional image {beat_index} for story beat (Pexels): {base_beat}")
                            found_image = True
                            break
                if not found_image:
    ...
    def run_hybrid_generation(self) -> Dict[str, List[str]]:
        ...
        for post in posts:
            ...
            # Step 3-5: Try sources in order
            images_found = []
            
            # Try Unsplash first
            unsplash_results = self.search_unsplash(query, self.images_per_post)
            for result in unsplash_results:
                image_url = result.get('urls', {}).get('regular', '')
                if image_url:
                    processed_path = self.download_and_process_image(image_url, "unsplash")
            ...
            if len(images_found) < self.images_per_post:
                pexels_results = self.search_pexels(query, self.images_per_post - len(images_found))
                for result in pexels_results:
                    image_url = result.get('src', {}).get('large', '')
                    if image_url:
                        processed_path = self.download_and_process_image(image_url, "pexels")
